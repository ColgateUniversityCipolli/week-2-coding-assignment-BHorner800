...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hldef{right.points} \hlkwb{<-} \hldef{a} \hlopt{+} \hldef{(a}\hlopt{+}\hlnum{1}\hldef{)}\hlopt{:}\hldef{n.rect}\hlopt{*}\hldef{(delta.x)}
  ...
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidial"}\hldef{)\{}
    \hldef{left.trap} \hlkwb{=} \hldef{left.points}
    \hldef{right.trap} \hlkwb{=} \hldef{right.points}
    \hldef{area} \hlkwb{=} \hlkwd{sum}\hldef{((}\hlkwd{integrand}\hldef{(left.trap)} \hlopt{+} \hlkwd{integrand}\hldef{(right.trap))}\hlopt{*}\hldef{(delta.x)}\hlopt{/}\hlnum{2}\hldef{)}
